...
## Команды и пути для пользователя — правила

- **Указывать абсолютные пути** ИЛИ явно говорить «выполнить из папки X» перед командой. Пользователь может быть в любом подкаталоге репо — относительный путь без контекста = команда не работает (свежий пример: `cp ... video-edits/_utils/...` из `~/teamplaner2/video-edits/` → ищется `video-edits/video-edits/_utils/`, не существует).
- **Одна команда — одна строка**. Mac Terminal склеивает многострочные блоки → аргументы съезжают.
- **Никогда `git push` без 3 проверок**: правильная ветка / auth настроен / нет конфликтов.

## Ветки и репо

- **Работаем на `main`** (cloud branch слита 2026-05-19).
- `claude/video-editing-montage-CAQlQ` — устарела, дубликат main. Можно удалить через `git push origin --delete claude/video-editing-montage-CAQlQ` если мешает.
- Бинарные файлы (`*.mp4 *.mov *.pt *.ogg *.heic` и др.) помечены в `.gitattributes` как `-text -diff` — больше не будет merge conflicts на них.
