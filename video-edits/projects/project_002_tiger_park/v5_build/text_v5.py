...
parts = []
FADE = 0.25  # 250ms fade-in for each text event
for (t0, t1, txt, font, size, color, x, y, opts) in texts:
    esc = escape_text(txt)
    # Fade-in alpha: 0 → 1 over FADE seconds starting at t0
...
# v5 color grade: teal-orange cinematic (cool shadows, warm highlights)
chain = ",".join(parts) + (
    ",eq=contrast=1.10:saturation=1.10"
    ",colorbalance=rs=-0.04:bs=0.05:rm=0.03:bm=-0.04:rh=0.05:bh=-0.05"
)
with open('/tmp/v5_tiger_text.txt', 'w') as f:
    f.write(chain)
