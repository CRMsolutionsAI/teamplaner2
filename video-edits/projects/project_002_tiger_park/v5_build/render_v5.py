...
shots = [
    # M1 Hook (0-6) — v5: stronger opener with torso close-up (visceral)
    (0.0,   3.0,  "video_CENTER", "IMG_torso_close.mov", 1, False),     # CHANGED: tiger torso BIG
    (3.0,   6.0,  "BLACK",        None, None, False),
    # M2 Setting (6-22)
    (6.0,   9.0,  "video_RIGHT",  "IMG_3478_tiger_5s.mp4", 0,  False),
    (9.0,   12.0, "video_CENTER", "IMG_pool_wide.mov", 0, False),
    (12.0,  16.0, "BLACK",        None, None, False),
    (16.0,  19.0, "video_LEFT",   "IMG_3489.mp4", 5,  False),
    (19.0,  22.0, "video_RIGHT",  "IMG_3489.mp4", 80, False),            # CHANGED: was duplicate
    # M3 Process (22-44)
    ...
    # M6 Climax (78-95) — v5: belly photo back (with motion), slow-mo on pool
    (78.0,  83.0, "video_CENTER", "IMG_3489.mp4", 150, False),
    (83.0,  87.0, "photo_zoom",   "tiger_belly_paws_up.jpeg", None, False),  # ⭐ photo with slow zoom (real belly)
    (87.0,  92.0, "BLACK",        None, None, False),
    (92.0,  95.0, "video_slowmo", "IMG_pool_swim.mov", 0, False),            # slow-mo pool (mystical)
    # M7 Insight (95-108.2)
...
def render_segment(i, t0, t1, kind, src, trim, crop_face):
    ...
    elif kind.startswith("video_") and kind.split("_")[1] in LAYOUTS:
    ...
    elif kind == "photo_zoom":
        # Photo with slow zoom-in (zoompan) — makes static photo less "cheap"
        L = LAYOUTS["CENTER"]
        src_path = f"{PHOTOS}/{src}"
        d = int(duration * 30)
        vf = (
            f"scale={L['w']*2}:{L['h']*2}:force_original_aspect_ratio=increase,"
            f"crop={L['w']*2}:{L['h']*2},"
            f"zoompan=z='1.0+0.08*on/{d}':d=1:"
            f"x='iw/2-iw/zoom/2':y='ih/2-ih/zoom/2':s={L['w']}x{L['h']}:fps=30,"
            f"format=yuv420p"
        )
        cmd = [
            "ffmpeg", "-y", "-hide_banner", "-loglevel", "error",
    ...
    elif kind == "video_slowmo":
        # Video at 50% speed — stretches source 1s → 2s visually
        L = LAYOUTS["CENTER"]
        src_path = f"{FOOTAGE}/{src}"
        # We have `duration` seconds to fill at 0.5x → need duration/2 seconds of source
        src_dur = duration / 2.0
        vf = (
            f"trim=duration={src_dur},setpts=PTS*2.0,fps=30,"
            f"scale={L['w']}:{L['h']}:force_original_aspect_ratio=increase,crop={L['w']}:{L['h']},"
            f"format=yuv420p"
        )
        cmd = [
