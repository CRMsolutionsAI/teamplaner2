"""v11 render — v10 + static photo_frame (no shake), text-card finale on black."""
import subprocess
import os
...
shots = [
    ...
    (46.5, 48.62,  "black", None, None, None),  # CTA: pure black for big text question (user request)
]
...
def render_segment(i, t0, t1, kind, src, p1, p2):
    ...
    if kind == "black":
        ...
        # Static JPEG/photo — NO motion (user said photos shake with zoompan)
        cmd = [
            "ffmpeg", "-y", "-hide_banner", "-loglevel", "error",
            "-loop", "1", "-t", f"{duration}", "-i", src,
            "-f", "lavfi", "-t", f"{duration}", "-i", f"color=c=black:s={W}x{H}:r=30",
