"""v10 render — v9 + new Bangkok footage + lifestyle product photos."""
import subprocess
import os
...
LIFESTYLE = f"{P}/sources/photos_new_v10"
...
shots = [
    # M2 (6.36-18.94) Night market — Bangkok atmosphere with new footage + stocks
    (6.36, 9.08,  "video_frame",       f"{FOOTAGE}/IMG_5642_route66_night.mov", 2.0, 1.0),       # ROUTE66 night crowd
    ...
    (13.49, 15.12, "video_frame",      f"{STOCK}/nm_28515508_woman_food_stand.mp4", 0.3, 1.0),   # STOCK: woman vendor
    (15.12, 17.59, "video_frame",      f"{FOOTAGE}/IMG_5644_transit.mov", 1.0, 1.0),             # WIPOUT ferris/neon
    (17.59, 18.94, "video_frame",      f"{FOOTAGE}/IMG_1110.mp4", 0, 1.1),                       # back to product
    # M3 (18.94-32.7) Process — magical restaurant + lifestyle photos
    (18.94, 20.77, "video_frame",      f"{FOOTAGE}/IMG_1110.mp4", 0.2, 1.1),                     # мастер
    (20.77, 22.5,  "video_frame",      f"{FOOTAGE}/IMG_1756_magical_restaurant.mov", 6.0, 1.0),  # +МАГИЯ ← flower restaurant
    (22.5, 24.0,   "video_frame",      f"{FOOTAGE}/IMG_0468.mp4", 1.0, 1.15),                    # тактильно
    (24.0, 25.42,  "video_frame",      f"{FOOTAGE}/IMG_0506.mp4", 0.5, 1.2),                     # надёжно
    (25.42, 27.5,  "photo_frame",      f"{LIFESTYLE}/lifestyle_green_natalia.jpeg", None, None), # NEW: real product in hand
    (27.5, 29.8,   "video_frame",      f"{FOOTAGE}/IMG_0932.mp4", 5.0, 1.4),                     # finishing
    (29.8, 32.7,   "photo_frame",      f"{LIFESTYLE}/lifestyle_black_natalia.jpeg", None, None), # NATALIA name reveal — real
    # M4 (32.7-40.88) Value/120
    ...
    (38.5, 40.88,  "photo_frame",      f"{LIFESTYLE}/lifestyle_burgundy_compass.jpeg", None, None), # NEW: burgundy compass — unique variant
    # M5 (40.88-48.6) Finale
    (40.88, 43.14, "photo_frame",      f"{LIFESTYLE}/lifestyle_green_natalia.jpeg", None, None), # ЭМОЦИЯ — real product
    (43.14, 44.7,  "video_frame",      f"{FOOTAGE}/IMG_5635_bangkok_scooter.mov", 3.0, 1.0),     # ВАШ ПАСПОРТ — your journey
    (44.7, 46.5,   "photo_frame",      f"{LIFESTYLE}/lifestyle_burgundy_compass.jpeg", None, None), # — ваша история
    (46.5, 48.62,  "png_two_covers_finale", None, None, None),                                   # CTA: 2 cover layout
]
...
def render_segment(i, t0, t1, kind, src, p1, p2):
    ...
    if kind == "black":
        ...
        # Static JPEG/photo scaled to FW x FH frame centered on black, subtle zoom
        cmd = [
            "ffmpeg", "-y", "-hide_banner", "-loglevel", "error",
            "-loop", "1", "-t", f"{duration}", "-i", src,
            "-f", "lavfi", "-t", f"{duration}", "-i", f"color=c=black:s={W}x{H}:r=30",
            "-filter_complex",
            (f"[0:v]scale={FW}:{FH}:force_original_aspect_ratio=increase,crop={FW}:{FH},"
             f"zoompan=z='1.0+0.05*on/{int(duration*30)}':"
             f"d=1:x='iw/2-iw/zoom/2':y='ih/2-ih/zoom/2':s={FW}x{FH}:fps=30,"
             f"format=yuv420p[fg];"
